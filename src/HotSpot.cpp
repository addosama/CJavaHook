...
int HotSpot::Array_u2::get_length()
{
    ...
    static VMStructEntry* _length_entry = find_VMStructEntry("Array<u2>", "_length", false);
    if (!_length_entry) return 0;
...
{
    static VMIntConstantEntry* entry = find_VMIntConstantEntry("JavaThreadState");
    if (!entry) return 0;
    return entry->value;
}
...
uint32_t HotSpot::Thread::get_suspend_flags()
{
    ...
    static VMStructEntry* _suspend_flags_entry = find_VMStructEntry("JavaThread", "_suspend_flags", false);
...
HotSpot::JavaThreadState HotSpot::Thread::get_thread_state()
{
    ...
    static VMStructEntry* _thread_state_entry = find_VMStructEntry("JavaThread", "_thread_state", false);
...
unsigned short* HotSpot::Method::get_flags()
{
    ...
    static VMStructEntry* _flags_entry = find_VMStructEntry("Method", "_flags", false);
...
void HotSpot::Method::set_dont_inline(bool enabled)
{
    ...
    static VMStructEntry* _flags_entry = find_VMStructEntry("Method", "_flags", false);
    if (!_flags_entry) return;

    unsigned short* flags = (unsigned short*)((uint8_t*)this + _flags_entry->offset);
    if (enabled)
        *flags |= _dont_inline;
    else
        *flags &= ~_dont_inline;
...
{
    static VMStructEntry* _method_entry = find_VMStructEntry("frame", "_method", false);
    if (!_method_entry) return nullptr;

    return *(Method**)((uint8_t*)this + _method_entry->offset);
}

// ---- FieldInfo, DirectoryEntry, Dictionary stubs ----
unsigned short HotSpot::FieldInfo::get_name_index()       { return _data[0]; }
unsigned short HotSpot::FieldInfo::get_signature_index()  { return _data[1]; }
unsigned short HotSpot::FieldInfo::get_access()           { return _data[2]; }
bool HotSpot::FieldInfo::is_internal()    { return _data[2] & JVM_ACC_FIELD_INTERNAL; }
bool HotSpot::FieldInfo::is_public()      { return _data[2] & JVM_ACC_PUBLIC; }
bool HotSpot::FieldInfo::is_private()     { return _data[2] & JVM_ACC_PRIVATE; }
bool HotSpot::FieldInfo::is_protected()   { return _data[2] & JVM_ACC_PROTECTED; }
bool HotSpot::FieldInfo::is_static()      { return _data[2] & JVM_ACC_STATIC; }
bool HotSpot::FieldInfo::is_final()       { return _data[2] & JVM_ACC_FINAL; }

HotSpot::DirectoryEntry* HotSpot::DirectoryEntry::get_next()
{
    static VMStructEntry* _next_entry = find_VMStructEntry("DirectoryEntry", "_next", false);
    if (!_next_entry) return nullptr;
    return *(DirectoryEntry**)((uint8_t*)this + _next_entry->offset);
}

void* HotSpot::DirectoryEntry::get_literal()
{
    static VMStructEntry* _literal_entry = find_VMStructEntry("DirectoryEntry", "_literal", false);
...
{
    static VMStructEntry* _buckets_entry = find_VMStructEntry("Dictionary", "_buckets", false);
...
{
    static VMStructEntry* _table_size_entry = find_VMStructEntry("Dictionary", "_table_size", false);
